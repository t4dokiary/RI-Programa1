...
from pyparsing import Word, alphas, oneOf, infixNotation, opAssoc, Literal, QuotedString
...
class SistemaRI:
    ...
    def crear_tabla(self, array, nombre):
        ...
        df = df.groupby(['Palabras']).size().reset_index(
            name='Frecuencia')  # Agrupamos las palabras dada a la frecuencia de veces que aparezcan
        ...
        df.to_csv('diccionario/' + nombre + '.csv', index=False)  # Conversion de la tabla a archivo de texto
            ...
            for archivo in os.listdir(self.directorio):  # Lectura de cada documento para ver la aparicion del termino
                ...
                with open(ruta, encoding="utf-8") as doc:
                    ...
                    for cnt in contenido:  # Comparacion del contendio del doc con el termino
                        if (p == cnt):
                            matriz_binaria[index_palabra][
                                index_documento] = 1  # Si hallamos la igualdad se agrega el 0 y terminamos for
                            break
    ...
    def not_function(self):
        ...
        self.interfaz.update_output(f"\n\t¬{docs_array}")
        self.impresion_postfijo.append(f"\n\t¬{docs_array}")
        resultado = [1 if docs == 0 else 0 for docs in binary_array]
        return resultado
...
def main():
    ...
    try:
        ...
        diccionario = [word for word in diccionario if
                       word not in stopwords.words("english") and word not in stopwords.words("spanish") and len(
                           word) > 2]
        ...
        df.to_csv('diccionario/Matriz_BIN.csv', index=False)  # Conversion de la tabla a archivo de texto
        ...
        while True:
            ...
            sistema_ri.query_stem_elements = [elemento for elemento in sistema_ri.query_stem_elements if
                                              elemento not in sistema_ri.grammar_elements]
...
# ejemplos ( ( perro | gato ) & ( ( ! ( delfin | oso ) ) &  lobo ) )
# ejemplos ( ( perro | gato ) & ( ( ! ( delfin ) | oso )  &  lobo ) )
